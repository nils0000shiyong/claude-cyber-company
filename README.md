...
- 不是一个部署型服务——它跑在你本地一台机器上，机器关了就停。
- 不是一个飞书集成方案——对接飞书的能力，飞书官方 CLI 做得更全（见下文「与飞书官方 CLI 的关系」），本项目的 Bot 只解决「入站实时触发」。
- 不是一个保证多 Agent 全自动协作的系统——链路能不能跑通，取决于任务复杂度和你的指令，终端交互式使用比飞书入口更可靠。
...
**与飞书官方 CLI 的关系**：飞书 2026 年发布了官方 [CLI](https://github.com/larksuite/cli)（200+ 命令 + Agent Skills，覆盖消息 / 文档 / 多维表格 / 表格 / 日历 / 邮箱等）。如果你要让 agent **主动操作**飞书——发消息、读写文档、上传下载文件——直接用官方 CLI。OPC 的 feishu-bot 解决的是另一半：**入站实时触发**，即用户发消息时唤醒你的 agent。两者可以组合：用 feishu-bot 接收触发，用飞书 CLI 执行飞书侧动作。

**访问控制与安全**：两层控制——飞书应用「可用范围」（平台层，决定谁能 @ 到 bot，不在范围内的人连消息都发不进来）+ 代码白名单（区分 owner / member）。⚠️ 当 `AGENT_ALLOW_UNSAFE_PERMISSIONS=true` 时，可用范围内的成员可借 bot 在宿主机执行命令。个人自用是安全的；多人协作前请评估是否收紧权限或加沙箱，**不要随意扩大飞书应用的可用范围**。
...
- **单机运行**：跑在你一台机器上，无高可用，依赖机器常开；每日推送等定时任务在机器休眠时不触发。
- **每条消息冷启动**：飞书 Bot 用 `claude -p` 单次调用，未使用 `--resume`，没有持久会话，上下文靠把历史拼进 prompt——慢，且拿不到 prompt cache 复用。
- **记忆有损**：跨消息记忆基于摘要压缩（≤300 字），长上下文会丢细节；记忆按会话存，不区分具体用户。
- **多 Agent 链路不保证跑通**：通过飞书的单次调用 + 超时限制下，Elon → 子 Agent → 验证的完整链路可能跑不完，终端交互式使用更可靠。
- **仅支持文本**：飞书 Bot 暂不处理图片 / 文件 / 语音。
- **每日推送依赖第三方**：数据源为非官方公开 API，可能失效，属非核心功能。
- **安全边界**：开启 `--dangerously-skip-permissions` 后，飞书应用可用范围内的成员等于拥有宿主机命令执行权——仅限信任成员使用。
